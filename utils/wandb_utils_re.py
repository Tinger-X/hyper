...
def check_wandb_resume(opt: "argparse.Namespace") -> "None|bool":
    ...
    if isinstance(opt.resume, str):
        if opt.resume.startswith(WANDB_ARTIFACT_PREFIX):
            if opt.global_rank not in [-1, 0]:  # For resuming DDP runs
                ...
                opt.weights = str(Path(model_dir) / "last.pt")
            return True
    return None


def process_wandb_config_ddp_mode(opt: "argparse.Namespace"):
...
class WandbLogger:
    def __init__(self, opt, name, run_id, data_dict, job_type="Training"):
        ...
        if isinstance(opt.resume, str):  # checks resume from artifact
            ...
            self.wandb_run = wandb.init(
                config=opt, resume="allow",
                project="YOLOv5" if opt.project == "runs/train" else Path(opt.project).stem,
                name=name, job_type=job_type, id=run_id
    ...
    def setup_training(self, opt, data_dict):
        ...
        if isinstance(opt.resume, str):
            m_dir, _ = self.download_model_artifact(opt)
            if m_dir:
                self.weights = Path(m_dir) / "last.pt"
                config = self.wandb_run.config
                opt.weights, opt.save_period = str(self.weights), config.save_period
                opt.batch_size, opt.bbox_interval = config.total_batch_size, config.bbox_interval
                opt.epochs, opt.hyper = config.epochs, config.opt["hyper"]
    ...
    def create_dataset_table(self, dataset, class_to_id, name="dataset"):
        ...
        class_set = wandb.Classes([{"id": _id, "name": name} for _id, name in class_to_id.items()])
    ...
    def log_training_progress(self, predn, path, names):
        if self.val_table and self.result_table:
            class_set = wandb.Classes([{"id": _id, "name": name} for _id, name in names.items()])
            box_data = []
            total_conf = 0
            ...
            _id = self.val_table_map[Path(path).name]
            self.result_table.add_data(
                self.current_epoch,
                _id,
                wandb.Image(self.val_table.data[_id][1], boxes=boxes, classes=class_set),
