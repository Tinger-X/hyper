...
def output_to_target(output):
    ...
    for i, o in enumerate(output):
        for *box, conf, cls in o.cpu().numpy():
            targets.append([i, cls, *list(*utils_re.xyxy2xywh(np.array(box)[None])), conf])
...
def plot_images(images, targets, paths=None, fname="images.jpg", names=None, max_size=640, max_subplots=16):
    ...
    for i, img in enumerate(images):
        ...
        if len(targets) > 0:
            ...
            boxes = utils_re.xywh2xyxy(image_targets[:, 2:6]).T
        ...
        if paths:
            ...
            t_size = cv2.getTextSize(label, 0, fontScale=tl / 3, thickness=tf)[0]
            cv2.putText(
                mosaic, label, (block_x + 5, block_y + t_size[1] + 5),
...
def plot_study_txt(path="", x=None):
    ...
    ax2.plot(
        ...
        [34.6, 40.5, 43.0, 47.5, 49.7, 51.5], "k.-",
        linewidth=2, markersize=8, alpha=.25, label="EfficientDet"
    )
...
def plot_labels(labels, names=(), save_dir=Path(""), loggers=None):
    ...
    labels[:, 1:] = utils_re.xywh2xyxy(labels[:, 1:]) * 2000
    ...
    for k, v in loggers.items() or {}:
        if k == "wandb" and v:
            # TODO: [Warning] Images sizes do not match. This will causes images to be display incorrectly in the UI.
