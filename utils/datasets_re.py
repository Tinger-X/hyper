...
from utils import g_var_re, torch_utils_re, utils_re
...
def create_dataloader(
        path, img_size, batch_size, stride, opt, hyper=None, augment=False,
        cache=False, pad=0.0, rect=False, rank=-1, world_size=1, workers=8,
        image_weights=False, quad=False, prefix="", sampler=None
):
...
def create_dataloader_rgb_ir(
        path1, path2, img_size, batch_size, stride, opt, hyper=None,
        augment=False, cache=False, pad=0.0, rect=False, rank=-1,
        world_size=1, workers=8, image_weights=False, quad=False, prefix="", sampler=None
):
    # Make sure only the first process in DDP process the dataset first, and the following others can use the cache
    with torch_utils_re.torch_distributed_zero_first(rank):
        dataset = LoadMultiModalImagesAndLabels(
            path1, path2, img_size, batch_size, augment=augment,
            hyper=hyper, rect=rect, cache_images=cache, single_cls=opt.single_cls,
...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources="streams.txt", img_size=640, stride=32):
        ...
        n = len(sources)
        self.imgs = [None] * n
        self.sources = [utils_re.clean_str(x) for x in sources]  # clean source names for later
        for i, s in enumerate(sources):  # index, source
            # Start thread to read frames from video stream
            print(f"{i + 1}/{n}: {s}... ", end="")
            if "youtube.com/" in s or "youtu.be/" in s:  # if source is YouTube video
                utils_re.check_requirements(("pafy", "youtube_dl"))
                import pafy
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(
            self, path, img_size=640, batch_size=16, augment=False, hyper=None,
        ...
        if exists:
            ...
            tqdm.tqdm(None, desc=prefix + d, total=n, initial=n)  # display cache results
        assert nf > 0 or not augment, (
            f"{prefix}: No labels in {cache_path}. "
            f"Can not train without labels. See {help_url}"
        )
    ...
    def cache_labels(self, path=Path("./labels.cache"), prefix=""):
        ...
        pbar = tqdm.tqdm(zip(self.img_files, self.label_files), desc="Scanning images", total=len(self.img_files))
                    ...
                    with open(lb_file, "r") as f:
                        ...
                        if any([len(x) > 8 for x in l]):  # is segment
                            ...
                            l = np.concatenate(
                                (
                                    classes.reshape(-1, 1),
                                    utils_re.segments2boxes(segments)),
                                1
                            )  # (cls, xywh)
    ...
    def __getitem__(self, index):
        ...
        hyper = self.hyper
        mosaic = self.mosaic and random.random() < hyper["mosaic"]
        ...
        if self.augment:
            ...
            if not mosaic:
                img, labels = random_perspective(
                    img, labels, degrees=hyper["degrees"], translate=hyper["translate"],
                    scale=hyper["scale"], shear=hyper["shear"], perspective=hyper["perspective"]
                )

            # Augment colorspace
            augment_hsv(img, hgain=hyper["hsv_h"], sgain=hyper["hsv_s"], vgain=hyper["hsv_v"])

        nL = len(labels)  # number of labels
        if nL:
            labels[:, 1:5] = utils_re.xyxy2xywh(labels[:, 1:5])  # convert xyxy to xywh
        ...
        if self.augment:
            # flip up-down
            if random.random() < hyper["flipud"]:
                img = np.flipud(img)
                if nL:
                    labels[:, 2] = 1 - labels[:, 2]

            # flip left-right
            if random.random() < hyper["fliplr"]:
                img = np.fliplr(img)
                if nL:
                    labels[:, 1] = 1 - labels[:, 1]
...
class LoadMultiModalImagesAndLabels(Dataset):  # for training/testing
    ...
    def __init__(
            self, path_rgb, path_ir, img_size=640, batch_size=16, augment=False, hyper=None, rect=False,
        ...
        try:
            ...
            assert len(self.img_files_rgb) > 0 and len(self.img_files_ir) > 0, f"{prefix}: No images found"
        ...
        cache_rgb_path = (
            p_rgb if p_rgb.is_file() else Path(self.label_files_rgb[0]).parent
        ...
        if cache_rgb_path.is_file():
            ...
            if cache_rgb["hash"] != get_hash(
                    self.label_files_rgb + self.img_files_rgb
            ) or "version" not in cache_rgb:  # changed
                cache_rgb, exists_rgb = self.cache_labels(
                    self.img_files_rgb, self.label_files_rgb, cache_rgb_path, prefix
                ), False  # re-cache
        else:
            cache_rgb, exists_rgb = self.cache_labels(
                self.img_files_rgb, self.label_files_rgb, cache_rgb_path, prefix
        ...
        cache_ir_path = (
            p_ir if p_ir.is_file() else Path(self.label_files_ir[0]).parent
        ).with_suffix(".cache")  # cached labels
        if cache_ir_path.is_file():
            ...
            if cache_ir["hash"] != get_hash(
                    self.label_files_ir + self.img_files_ir
            ) or "version" not in cache_ir:  # changed
                cache_ir, exists_ir = self.cache_labels(
                    self.img_files_ir, self.label_files_ir, cache_ir_path, prefix
                ), False  # re-cache
        else:
            cache_ir, exists_ir = self.cache_labels(
                self.img_files_ir, self.label_files_ir, cache_ir_path, prefix
        ...
        if exists_rgb:
            d = (
                f"Scanning RGB \"{cache_rgb_path}\" images and labels... {nf_rgb} found, "
                f"{nm_rgb} missing, {ne_rgb} empty, {nc_rgb} corrupted"
            )
            tqdm.tqdm(None, desc=f"{prefix}: {d}", total=n_rgb, initial=n_rgb)  # display cache results
        if exists_ir:
            d = (
                f"Scanning IR \"{cache_rgb_path}\" images and labels... {nf_ir} found, "
                f"{nm_ir} missing, {ne_ir} empty, {nc_ir} corrupted"
            )
            tqdm.tqdm(None, desc=f"{prefix}: {d}", total=n_ir, initial=n_ir)  # display cache results

        assert nf_rgb > 0 or not augment, (
            f"{prefix}: No labels in {cache_rgb_path}. "
            f"Can not train without labels. See {help_url}"
        )
    ...
    def cache_labels(self, imgfiles, labelfiles, path=Path("./labels.cache"), prefix=""):
        ...
        x = {}  # dict
        nm, nf, ne, nc = 0, 0, 0, 0  # number missing, found, empty, duplicate
        pbar = tqdm.tqdm(zip(img_files, label_files), desc="Scanning images", total=len(img_files))
        for i, (im_file, lb_file) in enumerate(pbar):
            try:
                # verify images
                im = Image.open(im_file)
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if labels.size:  # normalized xywh to pixel xyxy format
                labels[:, 1:] = utils_re.xywhn2xyxy(labels[:, 1:], ratio[0] * w, ratio[1] * h, padw=pad[0], padh=pad[1])
        ...
        if self.augment:
            # Augment colorspace
            augment_hsv(img_rgb, hgain=hyper["hsv_h"], sgain=hyper["hsv_s"], vgain=hyper["hsv_v"])
            augment_hsv(img_ir, hgain=hyper["hsv_h"], sgain=hyper["hsv_s"], vgain=hyper["hsv_v"])

        nL = len(labels_rgb)  # number of labels
        if nL:
            labels_rgb[:, 1:5] = utils_re.xyxy2xywh(labels_rgb[:, 1:5])  # convert xyxy to xywh
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        if labels.size:
            ...
            labels[:, 1:] = utils_re.xywhn2xyxy(labels[:, 1:], w, h, padw, padh)
            segments = [utils_re.xyn2xy(x, w, h, padw, padh) for x in segments]
    ...
    img4, labels4 = random_perspective(
        ...
        degrees=self.hyper["degrees"],
        translate=self.hyper["translate"],
        scale=self.hyper["scale"],
        shear=self.hyper["shear"],
        perspective=self.hyper["perspective"],
...
def load_mosaic_RGB_IR(self, index1, index2):
    ...
    for i, index in enumerate(indices):
        ...
        if i == 0:  # top left
            ...
            img4_rgb = np.full((s * 2, s * 2, img_rgb.shape[2]), 114, dtype=np.uint8)
            # base image with 4 tiles
            img4_ir = np.full((s * 2, s * 2, img_ir.shape[2]), 114, dtype=np.uint8)
            x1a, y1a, x2a, y2a = max(xc - w, 0), max(yc - h, 0), xc, yc  # xmin, ymin, xmax, ymax (large image)
            x1b, y1b, x2b, y2b = w - (x2a - x1a), h - (y2a - y1a), w, h  # xmin, ymin, xmax, ymax (small image)
        elif i == 1:  # top right
            x1a, y1a, x2a, y2a = xc, max(yc - h, 0), min(xc + w, s * 2), yc
        ...
        if labels_rgb.size:
            # normalized xywh to pixel xyxy format
            labels_rgb[:, 1:] = utils_re.xywhn2xyxy(labels_rgb[:, 1:], w, h, padw, padh)
            # normalized xywh to pixel xyxy format
            labels_ir[:, 1:] = utils_re.xywhn2xyxy(labels_ir[:, 1:], w, h, padw, padh)
            segments_rgb = [utils_re.xyn2xy(x, w, h, padw, padh) for x in segments_rgb]
            segments_ir = [utils_re.xyn2xy(x, w, h, padw, padh) for x in segments_ir]
    ...
    img4_rgb, img4_ir, labels4_rgb, labels4_ir = random_perspective_rgb_ir(
        img4_rgb, img4_ir, labels4_rgb, labels4_ir,
...
def load_mosaic9(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        if labels.size:
            # normalized xywh to pixel xyxy format
            labels[:, 1:] = utils_re.xywhn2xyxy(labels[:, 1:], w, h, padx, pady)
            segments = [utils_re.xyn2xy(x, w, h, padx, pady) for x in segments]
...
def random_perspective(
        img, targets=(), segments=(), degrees=10, translate=.1,
    ...
    if n:
        ...
        if use_segments:  # warp segments
            segments = utils_re.resample_segments(segments)  # upsample
            for i, segment in enumerate(segments):
                ...
                # clip
                new[i] = utils_re.segment2box(xy, width, height)
...
def random_perspective_rgb_ir(
        img_rgb, img_ir, targets_rgb=(), targets_ir=(), segments_rgb=(), segments_ir=(),
...
def cutout(image, labels):
    ...
    def bbox_ioa(box1, box2):
        ...
        # Intersection area
        area1 = (np.minimum(b1_x2, b2_x2) - np.maximum(b1_x1, b2_x1)).clip(0)
        area2 = (np.minimum(b1_y2, b2_y2) - np.maximum(b1_y1, b2_y1)).clip(0)
...
def flatten_recursive(path="../coco128"):
    ...
    for file in tqdm.tqdm(glob.glob(str(Path(path)) + "/**/*.*", recursive=True)):
...
def extract_boxes(path="../coco128/"):  # from utils.datasets import *; extract_boxes("../coco128")
    ...
    for im_file in tqdm.tqdm(files, total=n):
        if im_file.suffix[1:] in img_formats:
            ...
            if Path(lb_file).exists():
                ...
                for j, x in enumerate(lb):
                    ...
                    b[2:] = b[2:] * 1.2 + 3  # pad
                    b = utils_re.xywh2xyxy(b.reshape(-1, 4)).ravel().astype(int)
...
def autosplit(path="../coco128", weights=(0.9, 0.1, 0.0), annotated_only=False):
    ...
    Usage: from utils.datasets_re import *; autosplit("../coco128")
    Arguments
    ...
    print(f"Auto-splitting images from {path}" + ", using *.txt labeled images only" * annotated_only)
    for i, img in tqdm.tqdm(zip(indices, files), total=n):
