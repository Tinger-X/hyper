...
def create_dataloader_rgb_ir(
        path1, path2, imgsz, batch_size, stride, opt, hyp=None,
    ...
    with torch_distributed_zero_first(rank):
        dataset = LoadMultiModalImagesAndLabels(
            path1, path2, imgsz, batch_size, augment=augment,
...
class LoadImages:  # for inference
    def __init__(self, path, img_size=640, stride=32):
        p = str(Path(path))  # os-agnostic absolute path.absolute()
        if '*' in p:
...
class LoadMultiModalImagesAndLabels(Dataset):  # for training/testing
    ...
    def __init__(
            self, path_rgb, path_ir, img_size=640, batch_size=16, augment=False, hyp=None, rect=False,
