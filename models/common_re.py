...
from utils import utils_re, torch_utils_re, datasets_re, plots_re


__all__ = [
    "auto_pad", "DWConv",
    "Conv", "TransformerLayer", "TransformerBlock",
    "Bottleneck", "BottleneckCSP", "C3", "C3TR", "SPPF", "SPP",
    "Focus", "Contract", "Expand", "Concat", "Add", "ConvFusion",
    "Add2", "NMS", "AutoShape", "Detections", "Classify", "SelfAttention",
    "TransformerBlock2", "GPT", "spatial_shift1", "spatial_shift2",
    "SplitAttention", "S2Attention", "S2Block", "PreNormResidual"
]
...
class NMS(nn.Module):
    ...
    def forward(self, x):
        return utils_re.non_max_suppression(x[0], conf_thres=self.conf, iou_thres=self.iou, classes=self.classes)
...
class AutoShape(nn.Module):
    ...
    def forward(self, imgs, size=640, augment=False, profile=False):
        ...
        shape1 = [utils_re.make_divisible(x, int(self.stride.max())) for x in
                  np.stack(shape1, 0).max(0)]  # inference shape
        x = [datasets_re.letterbox(im, new_shape=shape1, auto=False)[0] for im in imgs]  # pad
        x = np.stack(x, 0) if n > 1 else x[0][None]  # stack
        x = np.ascontiguousarray(x.transpose((0, 3, 1, 2)))  # BHWC to BCHW
        x = torch.from_numpy(x).to(p.device).type_as(p) / 255.  # uint8 to fp16/32
        t.append(torch_utils_re.time_synchronized())
        ...
        with amp.autocast(enabled=p.device.type != "cpu"):
            ...
            # Post-process
            y = utils_re.non_max_suppression(y, conf_thres=self.conf, iou_thres=self.iou, classes=self.classes)  # NMS
            for i in range(n):
                utils_re.scale_coords(shape1, y[i][:, :4], shape0[i])
...
class Detections:
    ...
    def __init__(self, imgs, pred, files, times=None, names=None, shape=None):
        ...
        self.xywh = [utils_re.xyxy2xywh(x) for x in pred]  # xywh pixels
    ...
    def display(self, pprint=False, show=False, save=False, crop=False, render=False, save_dir=Path('')):
        for i, (im, pred) in enumerate(zip(self.imgs, self.pred)):
            string = f"image {i + 1}/{len(self.pred)}: {im.shape[0]}x{im.shape[1]} "
            if pred is not None:
                for c in pred[:, -1].unique():
                    n = (pred[:, -1] == c).sum()  # detections per class
                    string += f"{n} {self.names[int(c)]}{'s' * (n > 1)}, "  # add to string
                    ...
                    for *box, conf, cls in pred:  # xyxy, confidence, class
                        ...
                        if crop:
                            utils_re.save_one_box(
                                box,
                                im,
                                file=save_dir / 'crops' / self.names[int(cls)] / self.files[i]
                            )
                        else:  # all others
                            plots_re.plot_one_box(box, im, label=label, color=plots_re.colors(cls))
    ...
    def save(self, save_dir="runs/hub/exp"):
        # increment save_dir
        save_dir = utils_re.increment_path(save_dir, exist_ok=save_dir != "runs/hub/exp", mkdir=True)
...
class Classify(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1):  # ch_in, ch_out, kernel, stride, padding, groups
        ...
        self.conv = nn.Conv2d(c1, c2, k, s, auto_pad(k, p), groups=g)  # to x(b,c2,1,1)
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, attention_mask=None, attention_weights=None):
        ...
        :param x: tensor, input (token) dim:(b_s, nx, c)
        ...
        :return: output (tensor): dim:(b_s, nx, c)
        """
...
class GPT(nn.Module):
    ...
    def __init__(
            self, d_model, sr_ratio, vert_anchors, horz_anchors,
        ...
        self.trans_blocks = nn.Sequential(
            *[TransformerBlock2(d_model, d_k, d_v, h, block_exp, attn_pdrop, resid_pdrop, sr_ratio)
    ...
    def forward(self, x):
        ...
        # -------------------------------------------------------------------------
        rgb_fea_out = nn.functional.interpolate(rgb_fea_out, size=([h, w]), mode="bilinear")
        ir_fea_out = nn.functional.interpolate(ir_fea_out, size=([h, w]), mode="bilinear")
